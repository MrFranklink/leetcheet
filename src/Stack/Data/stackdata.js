...
import Stack from './Linked_list';

function CombinedComponenter() {
  const totalCheckboxes = Stack.length;
  ...
  const storageKey = 'stackComponentCheckedCheckboxes';

 
  const initialCheckedCheckboxes = JSON.parse(localStorage.getItem(storageKey)) || Array(totalCheckboxes).fill(false);
  const [checkedCheckboxes, setCheckedCheckboxes] = useState(initialCheckedCheckboxes);

  const handleCheckboxChange = (index) => {
    // Create a new array with the updated checkbox state
    ...
    localStorage.setItem(storageKey, JSON.stringify(newCheckedCheckboxes));
  };
  ...
  useEffect(() => {
    ...
    localStorage.setItem(storageKey, JSON.stringify(checkedCheckboxes));
  }, [checkedCheckboxes]);
...
export default CombinedComponenter;
